...
class ToolsVasController extends Controller
{
    public function importFilesVas(Request $request)
    {
        try {
            // ========================================================================================================================
            // Validation for the uploaded files
            // ========================================================================================================================
            if (!$request->hasFile('recon_file_vas')) {
                throw new \Exception('No file uploaded');
            }
            $file = $request->file('recon_file_vas');
            ...
            $sheet_data_recon->setCellValue('S1', 'PARAMETER_ALREADY_SUCCESS');
            $sheet_data_recon->setCellValue('T1', 'PARAMETER_STATUS_UPDATE');
            $sheet_format_gui_update->setCellValue('A1', 'ACTIVATION_ID');
            ...
            $sheet_data_recon->getStyle('P2:P' . $sheet_data_recon->getHighestRow())->getNumberFormat()->setFormatCode('yyyy-mm-dd hh:mm:ss');
            // ========================================================================================================================
            ...
            if ($sheet_data_recon->getHighestRow() >= 2) { // Check if there are at least 2 rows
                foreach ($sheet_data_recon->getRowIterator(2) as $row) { // Start from row 2 to skip header
                    ...
                    if (isset($already_success_lookup[$trx_id])) {
                        $sheet_data_recon->setCellValue('S' . $row->getRowIndex(), $already_success_lookup[$trx_id]); // Assuming PARAMETER_ALREADY_SUCCESS should be placed in column S
                    }
                }
            }
            ...
            // ========================================================================================================================
            // Iterate through DATA_RECON sheet and update DOM_TRX_DATE, statusCode, and datetime difference columns
            // ========================================================================================================================
            if ($sheet_data_recon->getHighestRow() >= 2) { // Check if there are at least 2 rows
                foreach ($sheet_data_recon->getRowIterator(2) as $row) { // Start from row 2 to skip header
                    $cellIterator = $row->getCellIterator();
                    ...
                    if (isset($dom_lookup[$trx_id])) {
                        ...
                        if ($selected_date !== null) {
                            ...
                            $parameter_already_success = $cells[18]; // Assuming PARAMETER_ALREADY_SUCCESS is in column S
                            if (empty($parameter_already_success) && $selected_status_code == "OK00") {
                                $sheet_data_recon->setCellValue('O' . $row->getRowIndex(), "SUKSES");
                                $sheet_data_recon->setCellValue('T' . $row->getRowIndex(), "SUKSES");
                            } else {
                                $sheet_data_recon->setCellValue('O' . $row->getRowIndex(), "GAGAL");
                                $sheet_data_recon->setCellValue('T' . $row->getRowIndex(), "GAGAL");
                            }
                        }
                    }
                }
            ...
            if ($sheet_data_recon->getHighestRow() >= 2) { // Check if there are at least 2 rows
                foreach ($sheet_data_recon->getRowIterator(2) as $row) { // Start from row 2 to skip header
                    ...
                    $activation_id = $cells[0]; // Assuming ACTIVATION_ID is in column A
                    $parameter_status_update = $cells[19]; // Assuming PARAMETER_STATUS_UPDATE is in column T

                    $sheet_format_gui_update->setCellValue('A' . $row_index, $activation_id);
                    $sheet_format_gui_update->setCellValue('B' . $row_index, $parameter_status_update);
                    $sheet_format_gui_update->setCellValue('C' . $row_index, $activation_id . '|' . $parameter_status_update);
            ...
            applyHeaderFormatting($sheet_data_recon, 'P1:R1', '00B050'); // Columns P to R with color 00B050
            applyHeaderFormatting($sheet_data_recon, 'S1', 'ED7D31'); // Columns W with color ED7D31
            applyHeaderFormatting($sheet_data_recon, 'T1', '002060', 'FFFFFF'); // Columns X to AA with color 002060 and text color FFFFFF

            $sheet_data_recon->setAutoFilter('A1:T' . $sheet_data_recon->getHighestRow());

            // ========================================================================================================================
            // Save the updated Excel file
            // ========================================================================================================================
            // Create CSV files
            $csv_vas_filename = 'FORMAT_UPDATE_GUI_VAS_' . date('Ymd') . '.csv';
